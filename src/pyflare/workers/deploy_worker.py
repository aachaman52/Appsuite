...
# ─── FTP Configuration (defaults to disabled; read from environment) ───
FTP_ENABLED_DEFAULT = False
FTP_HOST = os.environ.get("PYFLARE_FTP_HOST", "")
FTP_USER = os.environ.get("PYFLARE_FTP_USER", "")
FTP_PASS = os.environ.get("PYFLARE_FTP_PASS", "")
FTP_REMOTE_DIR = os.environ.get("PYFLARE_FTP_REMOTE_DIR", "/htdocs/builds")
WEB_BASE_URL = os.environ.get("PYFLARE_WEB_BASE_URL", "")
...
class DeployWorker(BaseWorker):
    ...
    def __init__(self, *args, output_dir, **kwargs):
        ...
        # FTP is strictly disabled by default unless explicitly enabled
        env_ftp_enabled = os.environ.get("PYFLARE_FTP_ENABLED", "false").lower() in ("1", "true", "yes")
        self.ftp_enabled = self.config.get("ftp_enabled", env_ftp_enabled)
    ...
    def run(self, job: Dict[str, Any], state: Any) -> 'WorkerResult':
        ...
        try:
            ...
            # Step 2: Deploy to FTP (if enabled)
            if not self.ftp_enabled or not self.ftp_host or not self.ftp_user:
                self.log.info("[%s] FTP deployment disabled; build saved locally to %s", job_id[:8], build_output_dir)
                return WorkerResult(
                    status=WorkerStatus.SUCCESS,
                    data={
                        "deployed": False,
                    ...
                    reason="FTP deployment disabled by default",
                    metadata={}
                )

            self.log.info("[%s] Starting FTP deployment to %s", job_id[:8], self.ftp_host)
