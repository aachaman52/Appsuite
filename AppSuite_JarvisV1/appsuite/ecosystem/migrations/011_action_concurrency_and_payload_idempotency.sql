-- ============================================================================
-- Migration 011: Action Concurrency Hardening and Request Fingerprint Idempotency
-- Project: pazkkzfdiwpcguoghlus (Aachman Studios Unified Supabase)
-- ============================================================================

-- 1. Enable pgcrypto extension if not already available for digest()
CREATE EXTENSION IF NOT EXISTS pgcrypto WITH SCHEMA extensions;

-- 2. Add request_fingerprint column to public.ecosystem_commands
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE public.ecosystem_commands ADD COLUMN request_fingerprint TEXT;
  END IF;
END $$;

-- 3. Replace public.execute_ecosystem_action RPC with hardened idempotency & concurrency
...
BEGIN
  ...
  v_request_fingerprint := encode(
    digest(
      p_action_type || ':' || COALESCE(p_payload, '{}'::jsonb)::text,
      'sha256'
    ),
    'hex'
  );
  ...
  IF p_idempotency_key IS NOT NULL AND trim(p_idempotency_key) <> '' THEN
    SELECT id, status, result, error_message, request_fingerprint, action_type
    INTO v_existing_id, v_existing_status, v_existing_result, v_existing_err, v_existing_fingerprint, v_existing_action_type
    ...
    IF v_existing_id IS NOT NULL THEN
      ...
      IF (v_existing_fingerprint IS NOT NULL AND v_existing_fingerprint <> v_request_fingerprint)
