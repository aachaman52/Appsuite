-- ============================================================================
-- Migration 012: Backfill Action Request Fingerprints and Replay Hardening
-- Project: pazkkzfdiwpcguoghlus (Aachman Studios Unified Supabase)
-- ============================================================================

-- 1. Enable pgcrypto extension if not already available
CREATE EXTENSION IF NOT EXISTS pgcrypto WITH SCHEMA extensions;

-- 2. Ensure idempotency_key and request_fingerprint columns and unique index exist
ALTER TABLE public.ecosystem_commands ADD COLUMN IF NOT EXISTS idempotency_key TEXT;
ALTER TABLE public.ecosystem_commands ADD COLUMN IF NOT EXISTS request_fingerprint TEXT;
...
-- Enable RLS and add SELECT policies so authenticated users can read their command and activity rows
ALTER TABLE public.ecosystem_commands ENABLE ROW LEVEL SECURITY;
GRANT SELECT ON public.ecosystem_commands TO authenticated;
GRANT SELECT ON public.user_activity TO authenticated;
GRANT SELECT, INSERT, UPDATE ON public.daymentor_user_data TO authenticated;
GRANT SELECT ON public.cricket_matches TO authenticated;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
      AND tablename = 'ecosystem_commands' 
      AND policyname = 'Users can view their own ecosystem commands'
  ) THEN
    CREATE POLICY "Users can view their own ecosystem commands"
    ON public.ecosystem_commands
    FOR SELECT
    TO authenticated
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
      ...
      AND policyname = 'Users can view their own user activity'
  ) THEN
...
-- 3. Backfill existing legacy rows that have an idempotency_key but NULL request_fingerprint

UPDATE public.ecosystem_commands
SET request_fingerprint =
    encode(
        extensions.digest(
            (action_type || ':' || COALESCE(payload, '{}'::jsonb)::text)::bytea,
            'sha256'
        ),
        'hex'
    )
...
-- 4. Drop legacy 2-arg execute_ecosystem_action overload to resolve PostgREST ambiguity
DROP FUNCTION IF EXISTS public.execute_ecosystem_action(TEXT, JSONB);

-- 5. Define public.record_user_activity function for ecosystem actions
...
-- 6. Replace public.execute_ecosystem_action RPC with fail-closed legacy fingerprint handling
...
BEGIN
  ...
  -- Compute deterministic request fingerprint (action_type + canonical JSONB text)
  v_request_fingerprint := encode(
    extensions.digest(
      (p_action_type || ':' || COALESCE(p_payload, '{}'::jsonb)::text)::bytea,
      'sha256'
    ),
    'hex'
  );
  ...
  IF p_idempotency_key IS NOT NULL AND trim(p_idempotency_key) <> '' THEN
    SELECT id, status, result, error_message, request_fingerprint, action_type, payload
    INTO v_existing_id, v_existing_status, v_existing_result, v_existing_err, v_existing_fingerprint, v_existing_action_type, v_existing_payload
    FROM public.ecosystem_commands
    WHERE user_id = v_user_id AND idempotency_key = p_idempotency_key;

    IF v_existing_id IS NOT NULL THEN
      -- If existing row had a legacy NULL fingerprint, compute what it would have been from stored payload
      IF v_existing_fingerprint IS NULL THEN
        v_existing_fingerprint := encode(
          extensions.digest(
            (v_existing_action_type || ':' || COALESCE(v_existing_payload, '{}'::jsonb)::text)::bytea,
            'sha256'
          ),
          'hex'
        );
        -- Backfill the stored fingerprint
        UPDATE public.ecosystem_commands
        SET request_fingerprint = v_existing_fingerprint
        WHERE id = v_existing_id;
      END IF;

      -- Replay conflict guard: same key with different action or payload is rejected
      IF (v_existing_fingerprint IS NULL OR v_existing_fingerprint <> v_request_fingerprint)
         OR (v_existing_action_type <> p_action_type) THEN
        RETURN jsonb_build_object(
          'success', false,
          'status', 'idempotency_conflict',
          'error', 'Idempotency conflict: action type or payload mismatch for this idempotency key.'
        );
      END IF;
    ...
    -- Claim command row in 'pending' state with request_fingerprint
    BEGIN
      INSERT INTO public.ecosystem_commands (
        user_id, target_app, action_type, payload, status, idempotency_key, request_fingerprint, created_at, completed_at
      ) VALUES (
        v_user_id, v_target_app, p_action_type, p_payload, 'pending', p_idempotency_key, v_request_fingerprint, now(), now()
  ...
  ELSE
    ...
    INSERT INTO public.ecosystem_commands (
      user_id, target_app, action_type, payload, status, request_fingerprint, created_at, completed_at
    ) VALUES (
      v_user_id, v_target_app, p_action_type, p_payload, 'pending', v_request_fingerprint, now(), now()
    ) RETURNING id INTO v_command_id;
  END IF;
  ...
  IF p_action_type = 'daymentor.create_task' THEN
    ...
    -- Strict validation (deterministic rejection, no silent default corruption)
    ...
    IF length(v_task_title) > 200 THEN
      UPDATE public.ecosystem_commands SET status = 'failed', error_message = 'Task title exceeds maximum allowed length of 200 characters.' WHERE id = v_command_id;
    ...
    IF v_task_priority NOT IN ('low', 'medium', 'high') THEN
      UPDATE public.ecosystem_commands SET status = 'failed', error_message = 'Invalid priority value. Must be low, medium, or high.' WHERE id = v_command_id;
      RETURN jsonb_build_object('success', false, 'error', 'Invalid priority value. Must be low, medium, or high.');
    END IF;
    IF v_task_difficulty NOT IN ('easy', 'medium', 'hard') THEN
      UPDATE public.ecosystem_commands SET status = 'failed', error_message = 'Invalid difficulty value. Must be easy, medium, or hard.' WHERE id = v_command_id;
      RETURN jsonb_build_object('success', false, 'error', 'Invalid difficulty value. Must be easy, medium, or hard.');
    END IF;
    ...
    -- Ensure base daymentor_user_data row exists
    INSERT INTO public.daymentor_user_data (
      user_id,
      data,
      updated_at
    ) VALUES (
    ...
    -- Row lock with FOR UPDATE to prevent concurrent lost updates
  ...
  ELSIF p_action_type = 'cricket_scorer.create_match' THEN
    ...
    IF lower(v_team_a) = lower(v_team_b) THEN
      UPDATE public.ecosystem_commands SET status = 'failed', error_message = 'Team A and Team B cannot be the same team.' WHERE id = v_command_id;
      RETURN jsonb_build_object('success', false, 'error', 'Team A and Team B cannot be the same team.');
    END IF;
    IF v_match_type NOT IN ('T20', 'ODI', 'Test') THEN
      UPDATE public.ecosystem_commands SET status = 'failed', error_message = 'Invalid match_type. Allowed: T20, ODI, Test.' WHERE id = v_command_id;
      RETURN jsonb_build_object('success', false, 'error', 'Invalid match_type. Allowed: T20, ODI, Test.');
    END IF;
    IF v_overs <= 0 OR v_overs > 100 THEN
      UPDATE public.ecosystem_commands SET status = 'failed', error_message = 'Overs must be a positive integer between 1 and 100.' WHERE id = v_command_id;
      RETURN jsonb_build_object('success', false, 'error', 'Overs must be a positive integer between 1 and 100.');
    END IF;
  ...
  ELSIF p_action_type = 'hackathon_simulator.start_simulation' THEN
    v_problem_id := trim(COALESCE(p_payload->>'problem_id', ''));
    v_problem_title := COALESCE(p_payload->>'problem_title', 'Hackathon Challenge');
    v_difficulty := COALESCE(p_payload->>'difficulty', 'medium');

    -- Strict validation
    IF length(v_problem_id) = 0 THEN
      UPDATE public.ecosystem_commands SET status = 'failed', error_message = 'problem_id is required.' WHERE id = v_command_id;
      RETURN jsonb_build_object('success', false, 'error', 'problem_id is required.');
    END IF;
    IF v_difficulty NOT IN ('easy', 'medium', 'hard') THEN
      UPDATE public.ecosystem_commands SET status = 'failed', error_message = 'Invalid difficulty. Allowed: easy, medium, hard.' WHERE id = v_command_id;
      RETURN jsonb_build_object('success', false, 'error', 'Invalid difficulty. Allowed: easy, medium, hard.');
    END IF;
