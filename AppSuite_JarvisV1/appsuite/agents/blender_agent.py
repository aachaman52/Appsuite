"""BlenderAgent — runs Blender-based model optimisation/import stages.
...
log = get_logger("agents.blender")


class BlenderAgent(BaseAgent):
    """Agent responsible for Blender import / model optimisation stage."""

    def receive_task(self, task: AgentTask, job_state: Optional[Dict[str, Any]] = None) -> None:
        log.info("[%s] Optimising models for: %s", self.name, task.objective)
        if self.message_bus:
    ...
    def execute_tools(self, plan: Any, job_state: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        Execute Blender pipeline nodes using the isolated ``job_state`` passed in.
