...
class GodotWorker(BaseWorker):
    ...
    def generate_main_scene(self, layout: Dict[str, Any], project_dir: Path) -> Path:
        ...
        if use_blender_scene:
            parts.append('[ext_resource type="PackedScene" path="res://Assets/scene.fbx" id="1_scene"]\n\n')
        ...
        for rel_res_path, rid in ext_resources.items():
            parts.append(f'[ext_resource type="PackedScene" path="{rel_res_path}" id="{rid}_scene"]\n\n')
            
        # Sky and Environment resources
        parts.append('[sub_resource type="ProceduralSkyMaterial" id="SkyMat"]\n')
        parts.append('sky_top_color = Color(0.384, 0.455, 0.549, 1)\n')
        parts.append('sky_horizon_color = Color(0.647, 0.655, 0.671, 1)\n\n')
        
        parts.append('[sub_resource type="Sky" id="SkyRes"]\n')
        parts.append('sky_material = SubResource("SkyMat")\n\n')
        
        parts.append('[sub_resource type="Environment" id="Env"]\n')
        parts.append('background_mode = 2\n')
        parts.append('sky = SubResource("SkyRes")\n')
        parts.append('ambient_light_source = 3\n')
        parts.append(f'ambient_light_color = Color(0.6, 0.7, 0.8, 1)\n')
        parts.append(f'ambient_light_energy = {ambient}\n\n')
        
        # Ground Material
        parts.append('[sub_resource type="StandardMaterial3D" id="GroundMaterial"]\n')
        parts.append('albedo_color = Color(0.2, 0.45, 0.25, 1)\n\n')
        
        parts.append('[node name="Main" type="Node3D"]\n\n')
        
        # Camera
        parts.append('[node name="Camera3D" type="Camera3D" parent="."]\n')
        parts.append('transform = Transform3D(1, 0, 0, 0, 0.7, 0.7, 0, -0.7, 0.7, 0, 40, 40)\n\n')
        
        # Ground with collision
        parts.append('[node name="Ground" type="StaticBody3D" parent="."]\n\n')
        parts.append('[node name="GroundMesh" type="CSGBox3D" parent="Ground"]\n')
        parts.append(f'size = Vector3({gsize}, 0.2, {gsize})\n')
        parts.append('material = SubResource("GroundMaterial")\n')
        parts.append('use_collision = true\n\n')
        
        # Lighting
        parts.append('[node name="Sun" type="DirectionalLight3D" parent="."]\n')
        parts.append('transform = Transform3D(1, 0, 0, 0, 0.5, 0.86, 0, -0.86, 0.5, 0, 50, 0)\n')
        parts.append(f'shadow_enabled = {str(lighting.get("shadows", True)).lower()}\n\n')
        parts.append('[node name="WorldEnvironment" type="WorldEnvironment" parent="."]\n')
        parts.append('environment = SubResource("Env")\n\n')
        
        # Blender Scene Instance
        if use_blender_scene:
            parts.append('[node name="BlenderScene" parent="." instance=ExtResource("1_scene")]\n\n')
        ...
        for obj in layout["objects"]:
            ...
            if (route == "direct_godot" or not use_blender_scene) and rel_res_path in ext_resources:
                ...
                parts.append(self._node(obj, hide_mesh=use_blender_scene))
    ...
    def run(self, job: Dict[str, Any], state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        scene = self.generate_main_scene(layout, project_dir)
