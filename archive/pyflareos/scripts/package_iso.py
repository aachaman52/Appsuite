...
class ISOPackager:
    def __init__(self, base_dir: Path, logger: logging.Logger, work_dir: Path = None):
        ...
        if config_path.exists():
            ...
            self.config = {}
        self.work_dir = work_dir if work_dir else (self.base_dir / self.config.get("build", {}).get("work_folder", "build"))
        self.extract_dir = self.work_dir / "iso_extracted"
        self.squashfs_dir = self.work_dir / "squashfs-root"
        self.output_dir = self.base_dir / self.config.get("build", {}).get("output_folder", "output")
...
""")
        ...
        cmd = [
            ...
            "--volid", label,
            "--appid", f"PyFlare OS v{self.config.get('os', {}).get('version', '1.0.0')}",
